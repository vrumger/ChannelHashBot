...
export default (bot: TBot, db: Database) => {
    bot.command(`watch`, adminMiddleware(), ctx => {
        ...
        db.channels.find({ admins: { $elemMatch: ctx.from!.id } }, (err: Error, channels: Channel[]) => {
            if (err) {
                console.error(err);
    ...
    bot.action(/^(\d+):(-\d+):(-?\d+)$/, ctx => {
        ...
        db.groups.findOne({ chat_id: group }, (err, chat) => {
            ...
            db.groups.update({ chat_id: group }, { $set: { tags: tagsObject } }, { upsert: true });
    ...
    bot.action(/^(\d+):done$/, ctx => {
        const message_id = ctx.callbackQuery!.message?.reply_to_message?.message_id;
        const from = Number(ctx.match![1]);
